...
RUN apt update -y \
	...
	&& echo "$EXIM_IMAP__USER__STD:$EXIM_IMAP__USER__STD_PASS" | chpasswd

USER $EXIM_IMAP__USER__SYS
COPY $EXIM_IMAP__CONF__CORE_DEST $EXIM_IMAP__CONF__CORE_SOURCE
COPY $EXIM_IMAP__CONF__OVERRIDES_DEST $EXIM_IMAP__CONF__OVERRIDES_SOURCE
